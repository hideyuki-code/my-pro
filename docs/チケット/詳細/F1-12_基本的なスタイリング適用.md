...
## 口頭で説明できるようにすべき質問
-   `tailwind.config.js` でカスタムカラーやカスタムフォントを定義するメリットは何ですか？
-   Tailwind CSSの `@apply` ディレクティブはどのような場合に使用しますか？メリットとデメリットは？
-   基本的なレスポンシブデザインの考え方（モバイルファーストなど）と、それをTailwind CSSでどのように実現しますか？
-   アプリケーション全体で一貫したスペーシング（マージンやパディング）を保つための工夫はありますか？（Tailwindのデフォルト値の活用など）
-   これまでのチケットで作成したコンポーネントの中で、特にスタイリングの調整が必要だと感じた箇所とその理由は何ですか？
